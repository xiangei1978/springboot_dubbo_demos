...
public class RocketMQTestController {
    ...
    @ApiImplicitParams({
            @ApiImplicitParam(paramType = "query", name = "topic", value = "topic名称", defaultValue = "TopicTest1", dataType = "string"),
            @ApiImplicitParam(paramType = "query", name = "tag", value = "tag名称", defaultValue = "TagA", dataType = "string")
    })
    @RequestMapping(value="/sendMsg_TagA", method= RequestMethod.POST)
    public CommonResult sendMsg_TagA(String topic,String tag){
    ...
    private SendResult sendMsg_withreturn(Message msg) {
//        Message msg = new Message("TopicTest1", // topic
//                "TagA", // tag
//                "OrderID00" + i, // key
//                ("Hello zebra mq" + i).getBytes());// body
        try {
            SendResult r= defaultProducer.send(msg );

            System.out.println("sendMsg_withreturn: \n" +
                    "msg = " + msg.toString() + "\n" +
                    "sendResult = " + r + "\n\n");
            i++;
